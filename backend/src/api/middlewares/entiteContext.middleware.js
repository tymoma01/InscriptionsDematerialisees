...
// Même repli pour un hostname complet listé dans HOTES_ENTITE_PAR_DEFAUT (voir config/env.js) :
// nécessaire pour le domaine personnalisé de prod (ex. "inscriptions.accecit.com"), dont le
// premier label ne correspond à aucun des cas ci-dessous ni à un code d'entité réel — cette
// instance ne servant qu'ACCECIT, il n'y a pas d'ambiguïté à résoudre par sous-domaine ici.
async function entiteContext(req, res, next) {
  try {
    const sousDomaine = req.hostname.split('.')[0];
    const utiliserEntiteParDefaut =
      ['localhost', 'www', req.hostname].includes(sousDomaine) || HOTES_ENTITE_PAR_DEFAUT.includes(req.hostname);
    const codeEntite = utiliserEntiteParDefaut ? ENTITE_PAR_DEFAUT : sousDomaine;
