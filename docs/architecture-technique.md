...
- `backend/src/integrations/notifications/NotificationProvider.js` : interface commune (`envoyer(destinataire, canal, message, options)`), sur le même principe que `StorageConnector` — pensée pour qu'un futur changement de prestataire SMS/email ne modifie qu'un fichier. `options.sujet`/`options.piecesJointes` (2026-07-30) : ajoutés pour la convocation au test (email + `.ics` joint, voir `invitationTestService.js`) — **forme non vérifiée auprès d'AllMySMS**, à confirmer avant la mise en production réelle (voir le commentaire dédié dans `allMySmsProvider.js`).
- `backend/src/integrations/notifications/allMySmsProvider.js` : unique implémentation actuelle, réutilisant le compte AllMySMS déjà existant (pas de nouveau prestataire ouvert, conformément à CLAUDE.md). Identifiants (`ALLMYSMS_API_LOGIN`/`ALLMYSMS_API_PASSWORD`) non encore récupérés à ce stade (2026-07-30) — le code est structuré et testé en amont de leur disponibilité, mais aucun envoi réel n'est possible tant qu'ils ne sont pas renseignés.
- `backend/src/integrations/notifications/generateurIcs.js` (2026-07-30, participants + horaire UTC corrigés le 2026-08-04) : génère le fichier `.ics` joint à la convocation (lib `ics`), durée de créneau par défaut 1h (`rendezvous.date_heure` ne porte qu'un instant de départ), lieu en dur (adresse ACCECIT déjà affichée en pied de page back-office) faute de champ "lieu" dédié sur `rendezvous`/`entites`. `DTSTART` encodé en UTC (`startOutputType: 'utc'`, suffixe `Z`) — la version initiale encodait une heure "flottante" Europe/Paris (aucun suffixe/`TZID`), que tout client calendrier interprète dans SON PROPRE fuseau plutôt que Paris ; corrigé après inspection de la sortie brute (`ics` ne propose que 'local' flottant ou 'utc' absolu dans cette version, pas de `VTIMEZONE`). `attendees` : candidat systématiquement (si son email est connu), formateur/inspecteur assigné en plus si `rendezvous.formateur_id` est renseigné (lookup dans `invitationTestService.js` via `utilisateurRepository.trouverUtilisateurParId`) — absent d'un rendez-vous pas encore assigné. `organizer` volontairement omis (voir point ouvert ci-dessous) plutôt qu'à moitié renseigné : sans `email`, `ics` produit une ligne `ORGANIZER` sans valeur après les deux-points, invalide en iCalendar (RFC 5545).
...
- **Point ouvert (2026-08-04, à remonter à Thomas) : ni l'adresse d'expédition email, ni l'expéditeur SMS ne sont documentés ou configurés nulle part** — ni en dur, ni en variable d'environnement, ni en Key Vault. Le payload envoyé à AllMySMS (`allMySmsProvider.js`) ne porte que `login`/`password`/destinataire/message, aucun champ `from`/`expediteur`/`sender_id` ; l'identité d'expédition dépend donc entièrement d'un réglage côté compte AllMySMS jamais vérifié ni documenté ici. C'est la même adresse manquante qui empêche de renseigner `organizer.email` dans `generateurIcs.js` ci-dessus. À trancher avant mise en production, indépendamment du chantier participants/attendees ci-dessus (hors périmètre de ce chantier).
