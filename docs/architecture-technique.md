...
### 3.3 AllMySMS (sms) + Microsoft Graph (email)

- `backend/src/integrations/notifications/NotificationProvider.js` : interface commune (`envoyer(destinataire, canal, message, options)`), sur le même principe que `StorageConnector` — pensée pour qu'un futur changement de prestataire SMS/email ne modifie qu'un fichier. `options.sujet`/`options.piecesJointes` (2026-07-30) : ajoutés pour la convocation au test (email + `.ics` joint, voir `invitationTestService.js`).
- **Décision du 2026-08-05 : AllMySMS ne couvre plus le canal email, seulement le SMS.** Vérification faite auprès de la documentation officielle AllMySMS : ce prestataire ne propose pas d'envoi d'email réel — leur seul service proche, Mail2SMS, fait l'inverse (convertir un email reçu en SMS). Le canal email bascule donc sur Microsoft Graph (`backend/src/integrations/notifications/graphMailProvider.js`), en réutilisant l'app registration/les secrets Key Vault déjà en place pour SharePoint/OneDrive (`graph-client-id`/`graph-client-secret`/`graph-tenant-id`, voir §2) — permission d'application supplémentaire requise : `Mail.Send` (consentement admin), en plus de `Files.ReadWrite.All` déjà accordée pour le stockage. Boîte d'expédition : `inscriptions@accecit.com` (`POST /v1.0/users/inscriptions@accecit.com/sendMail`), en dur pour ACCECIT comme `NOM_BIBLIOTHEQUE` dans `azureOneDriveConnector.js` — à faire évoluer vers une valeur de configuration par entité si une autre entité utilise elle aussi Graph pour ses emails.
- `backend/src/integrations/notifications/notificationFactory.js` : ne renvoie plus directement `allMySmsProvider`, mais un petit dispatcher qui route par `canal` (`'email'` → `graphMailProvider`, `'sms'` → `allMySmsProvider`) — les appelants (`invitationTestService.js`, `relanceService.js`, `rappelService.js`) n'ont pas changé, ils appellent toujours `notificationFactory()` une seule fois puis `.envoyer(destinataire, canal, ...)` avec un canal qui varie d'un appel à l'autre.
- `backend/src/integrations/stockage/erreursGraph.js` : `traduireErreurGraph` accepte désormais un troisième paramètre optionnel `{ permission }` (défaut `"Files.ReadWrite.All"`, surchargé en `"Mail.Send"` par `graphMailProvider.js`) pour que le message d'erreur 403 cite la bonne permission Graph selon l'appelant ; code `ErrorAccessDenied` (observé sur `/sendMail`) ajouté au même titre qu'`AccessDenied`/`Forbidden` (API fichiers).
- `backend/src/integrations/notifications/allMySmsProvider.js` : reste l'implémentation du canal SMS uniquement désormais. Identifiants (`ALLMYSMS_API_LOGIN`/`ALLMYSMS_API_PASSWORD`) non encore récupérés à ce stade (2026-07-30) — aucun envoi SMS réel n'est possible tant qu'ils ne sont pas renseignés.
- `backend/src/integrations/notifications/generateurIcs.js` (2026-07-30, participants + horaire UTC corrigés le 2026-08-04) : génère le fichier `.ics` joint à la convocation (lib `ics`), durée de créneau par défaut 1h (`rendezvous.date_heure` ne porte qu'un instant de départ), lieu en dur (adresse ACCECIT déjà affichée en pied de page back-office) faute de champ "lieu" dédié sur `rendezvous`/`entites`. `DTSTART` encodé en UTC (`startOutputType: 'utc'`, suffixe `Z`) — la version initiale encodait une heure "flottante" Europe/Paris (aucun suffixe/`TZID`), que tout client calendrier interprète dans SON PROPRE fuseau plutôt que Paris ; corrigé après inspection de la sortie brute (`ics` ne propose que 'local' flottant ou 'utc' absolu dans cette version, pas de `VTIMEZONE`). `attendees` : candidat systématiquement (si son email est connu), formateur/inspecteur assigné en plus si `rendezvous.formateur_id` est renseigné (lookup dans `invitationTestService.js` via `utilisateurRepository.trouverUtilisateurParId`) — absent d'un rendez-vous pas encore assigné. `organizer` toujours volontairement omis à ce stade (voir point ouvert ci-dessous, inchangé par le chantier du 2026-08-05) : sans `email`, `ics` produit une ligne `ORGANIZER` sans valeur après les deux-points, invalide en iCalendar (RFC 5545).
- Cas d'usage déclenchés par le moteur ou par la coordination : convocation au test (`invitationTestService.js`, appelée par `planificationRendezvousService.js` après validation de la transaction — planification initiale et replanification, jamais dans la même transaction que la création du rendez-vous), relance manuelle sms/email (`relanceService.js`, envoi réel depuis le 2026-07-30 — pour ces deux canaux, le champ `resultat` de `relances` n'est plus un choix de l'agent mais déterminé par le succès/échec technique de l'envoi ; `'telephone'` reste un choix libre, décrivant un appel passé hors de l'application), rappel automatique de créneau (`rappelService.js`, inchangé). Chaque envoi est journalisé dans `relances` (canal, utilisateur à l'origine, résultat) — ce qui répond directement au besoin terrain *"historique des relances par candidat, pour ne pas relancer en double"*.
- Comme pour SmartOF, l'activation est conditionnée à `req.entite.sms_actif` ; si `false`, le moteur n'invoque ni `allMySmsProvider` ni `graphMailProvider` (le booléen couvre encore les deux canaux, pas de champ `email_actif` distinct à ce stade).
- **Point ouvert restant (2026-08-04, à remonter à Thomas) — expéditeur SMS toujours non documenté/configuré** : le payload envoyé à AllMySMS (`allMySmsProvider.js`) ne porte que `login`/`password`/destinataire/message, aucun champ `from`/`expediteur`/`sender_id` ; l'identité d'expédition SMS dépend donc entièrement d'un réglage côté compte AllMySMS jamais vérifié ni documenté ici. Le volet email de ce point ouvert est résolu par la décision du 2026-08-05 ci-dessus (`inscriptions@accecit.com`) — cette même adresse pourrait aussi lever le blocage `organizer` de `generateurIcs.js`, mais ce changement n'a pas été demandé dans ce chantier et reste donc en l'état.
- **Point ouvert (2026-08-05, à remonter à Thomas) — DNS `accecit.com` non authentifié pour Microsoft 365, envoi réel non confirmé livré** : test d'envoi réel effectué le 2026-08-05 via `graphMailProvider.js` (`inscriptions@accecit.com` → une adresse Gmail personnelle) — Microsoft Graph répond `202 Accepted`, mais le message n'a été retrouvé ni en boîte de réception ni en spam côté destinataire (vérifié : recherche "Tous les messages" incluse). Cause probable identifiée par un contrôle DNS public sur `accecit.com` : le MX (`mailinblack.com`) et le SPF (`v=spf1 include:spf.mailinblack.com ~all`) ne couvrent que MailInBlack, jamais `include:spf.protection.outlook.com` (M365/Exchange Online) — aucun enregistrement DKIM M365 (`selector1`/`selector2._domainkey`) ni DMARC (`_dmarc.accecit.com`, NXDOMAIN) non plus, alors qu'un TXT `MS=ms92902825` confirme que `accecit.com` est bien un domaine vérifié dans un tenant M365. Un email envoyé par ce tenant échoue donc l'alignement SPF/DKIM/DMARC aux yeux du destinataire, ce qui peut aller jusqu'au rejet silencieux à la réception (pas systématiquement un simple classement en spam) selon la politique du serveur destinataire. **Diagnostic non confirmable plus loin depuis l'application** : l'app registration ne porte que `Mail.Send`, pas `Mail.Read` (403 `ErrorAccessDenied` en tentant de lire Éléments envoyés/boîte de réception d'`inscriptions@accecit.com` pour vérifier un NDR) — un accès admin M365 (centre d'administration Exchange, Suivi des messages/Message trace) est nécessaire pour confirmer le statut réel de cet envoi. **À trancher/corriger avant toute mise en production du canal email** : ajouter `include:spf.protection.outlook.com` au SPF, configurer le DKIM M365, poser un DMARC — indépendamment du fait que le code de `graphMailProvider.js` lui-même est validé (permissions Graph correctes, payload accepté, tests unitaires verts).

### 3.4 Pourquoi cette isolation compte pour la réutilisabilité

Les deux intégrations vivent exclusivement sous `backend/src/integrations/`, jamais sous `backend/src/core/`. Un `grep` de "smartof", "allmysms" ou "graphMailProvider" dans `core/` doit renvoyer zéro résultat : c'est le test de non-régression le plus simple pour vérifier que l'isolation n'a pas été rompue au fil du développement.
