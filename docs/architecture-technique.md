...
- `backend/src/integrations/notifications/generateurIcs.js` (2026-07-30) : génère le fichier `.ics` joint à la convocation (lib `ics`), durée de créneau par défaut 1h (`rendezvous.date_heure` ne porte qu'un instant de départ), lieu en dur (adresse ACCECIT déjà affichée en pied de page back-office) faute de champ "lieu" dédié sur `rendezvous`/`entites`.
